...
class BedrockThrottlingError(Exception):
    """Raised when Bedrock consistently throttles requests."""
    pass


class BedrockAPIError(Exception):
    """Raised for non-throttling Bedrock API errors."""
    pass
...
class BedrockEmbeddings:
    ...
    def generate_embedding(self, text: str) -> List[float]:
        """
        Generate embedding for a single text with robust retry logic.

        Raises:
            BedrockThrottlingError: if repeated throttling is observed.
            BedrockAPIError: for other persistent API errors.
        """
        max_retries = 6
        base_delay = 2
        ...
        for attempt in range(max_retries):
            try:
                ...
                if not embedding:
                    raise BedrockAPIError('No embedding returned in response')
                ...
                is_throttle = 'Throttling' in error_code or 'ThrottlingException' in err_msg or 'TooManyRequests' in err_msg or '429' in str(e)
                ...
                if is_throttle and attempt < max_retries - 1:
                    ...
                    logger.warning(f"Bedrock throttled (code={error_code}), retrying in {total_delay:.1f}s (attempt {attempt + 1}/{max_retries})")
                    time.sleep(total_delay)
                    continue
                elif is_throttle:
                    logger.error(f"Bedrock throttling persisted after {max_retries} attempts: {err_msg}")
                    raise BedrockThrottlingError(err_msg)
                else:
                    logger.error(f"ClientError generating embedding: {error_code} - {err_msg}")
                    raise BedrockAPIError(err_msg)

            except Exception as e:
                # Handle generic exceptions with limited retries
                if attempt < max_retries - 1:
                    delay = base_delay * (2 ** attempt)
                    jitter = random.uniform(0, base_delay)
                    total_delay = delay + jitter
                    logger.warning(f"Transient error generating embedding, retrying in {total_delay:.1f}s (attempt {attempt + 1}/{max_retries}): {e}")
                    time.sleep(total_delay)
                    continue
                logger.error(f"Error generating embedding after retries: {e}")
                raise BedrockAPIError(str(e))
